 ...
 */
	...
	public function add() {
		$typeAlias = 'blog';
		$Blog = $this->{$this->modelClass};
		$type = $Blog->Taxonomy->Vocabulary->Type->findByAlias($typeAlias);

		if (!empty($this->request->data)) {
			/*if (isset($this->request->data[$Blog->alias]['type'])) {
				$typeAlias = $this->request->data[$Blog->alias]['type'];
				$Blog->type = $typeAlias;*/
			//}
			if ($Blog->saveNode($this->request->data, $typeAlias)) {
				/*Croogo::dispatchEvent('Controller.Nodes.afterAdd', $this, array('data' => $this->request->data));*/
				$this->Session->setFlash(__d('croogo', '%s has been saved', $type['Type']['title']), 'flash', array('class' => 'success'));
				return $this->redirect(array('plugin' => 'nodes', 'controller' => 'nodes', 'action' => 'promoted'));
			} else {
				$this->Session->setFlash(__d('croogo', '%s could not be saved. Please, try again.', $type['Type']['title']), 'flash', array('class' => 'error'));
			}
		} else {
			$this->Croogo->setReferer();
			$this->request->data[$Blog->alias]['user_id'] = $this->Session->read('Auth.User.id');
		}

		/*$this->set('title_for_layout', __d('croogo', 'Create content: %s', $type['Type']['title']));*/
		/*$Blog->type = $type['Type']['alias'];*/
		/*$Blog->Behaviors->attach('Tree', array(
			'scope' => array(
				$Blog->escapeField('type') => $Blog->type,
			),
		));*/
	...
	public function delete($id) {
    	...
    	if(!$userId){ 
            $this->Session->setFlash(__('Anonymous comment is only 
			deletable through admin panel', true)); 
            $this->redirect('/'); 
        } 
        
        if ($userId != $node['Node']['user_id']) {
        	 $this->Session->setFlash(__('Ce post ne vous appartient pas.'));
                return $this->redirect('/');
        }
